...
class GPSVis(object):
    ...
    def plot_map(self, output='save', save_as='resultMap.png'):
        ...
        ax.scatter(self.x_fail, self.y_fail, s=2, marker="x")
    ...
    def create_image(self, color, width=2):
        ...
        :return:
        """
        data = pd.read_csv(self.data_path, names=['ID','RSSI','LATITUDE', 'LONGITUDE'], sep=',')
        ...
        for i in range(len(img_points)):
            if img_points[i][2] != -150:
                ...
                self.x_fail.append(img_points[i][0])
                self.y_fail.append(img_points[i][1])
                self.z_fail.append(img_points[i][2])
