...
#   logging.info("send time at "+str(send_minute)+"m")
  ...
  while True:
    ...
    # while time_to_send:
    time_to_send = str(int(time.time()))
    time_message = time_to_send + "\r\n"
    time_ambi = str(datetime.datetime.fromtimestamp(int(time_to_send)).strftime('%Y-%m-%d %H:%M:%S'))
